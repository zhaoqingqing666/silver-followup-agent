...
| ID | 严重度 | 场景 | 一句话问题 | 状态 |
|---|---|---|---|---|
| RV-01 | P0 | 场景三 | 自然语言路径无法触发冲突检查，必须点按钮 | 已修复 `5ed0f81` |
| RV-02 | P0 | 场景三 | 冲突场景的种子数据与测试日期会过期，9/18 后无法复现 | 已修复 `5ed0f81` |
| RV-03 | P0 | 场景四 | 越界词表覆盖不足，未命中时被当作普通信息静默忽略 | 已修复 `5ed0f81` |
| RV-04 | P1 | 场景一 | `acceptAlternative` 问在选号之后，正常流程多一轮无意义询问 | 已修复 `5ed0f81` |
| RV-05 | P1 | 场景三 | 确认卡不体现"已检测到冲突"，用户保留冲突时无二次提示 | 已修复 `5ed0f81` |
| RV-06 | P1 | 场景三 | 「仍保留这个时间」被前端分页隐藏到第 2 页 | 已修复 `5ed0f81` |
| RV-07 | P1 | 场景四 | 越界不产生状态、无专属视觉；`BoundaryAlert` 组件未实现 | 已修复 `5ed0f81` |
| RV-08 | P1 | 全部 | 没有 `scenarioId` 或场景重置入口，四场景无法按需稳定复现 | 已修复 `5ed0f81` |
| RV-09 | P2 | — | `respondWithCancelCard()` 为从未调用的死代码 | 已修复 `5ed0f81` |
| RV-10 | P2 | — | `sideTask` / `returnPolicy` 只写不读，却参与持久化快照 | 已修复 `5ed0f81` |
| RV-11 | P2 | 场景三 | 冲突检查窗口硬编码 60 分钟，未用科室实际时长 | 已修复 `5ed0f81` |
| RV-12 | P2 | 场景二 | `queryAlternatives` 窗口含当天，"附近日期"混入"同日其它时段" | 已修复 `5ed0f81` |
