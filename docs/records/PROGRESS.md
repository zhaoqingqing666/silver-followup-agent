...
## 2026-09-11 四场景审阅问题清单

- 新增 [四场景审阅问题清单](REVIEW_FINDINGS.md)，逐场景记录正常办理、预约失败、时间冲突、服务越界的实际代码路径与待修问题，共 12 项（P0 三项、P1 五项、P2 四项），每项附现象、根因、`文件:行号` 证据和建议改法。
- P0：自然语言路径无法触发冲突检查（RV-01）；冲突场景种子数据与测试日期 2026-09-18 后失效（RV-02）；越界词表覆盖不足导致部分医疗提问被静默忽略（RV-03）。
- 同文件附"已确认无问题的部分"，列出后续修改不应破坏的既有性质（确认门禁集中、冲突检查只读、无号源不抛异常、拒绝替代日期不查候选等）。
- 本次仅静态代码阅读，未启动后端、未连接 Dev Container、未执行 `mvn test` 或前端构建；上述结论不代表端到端验收结果。业务代码与接口未修改。
