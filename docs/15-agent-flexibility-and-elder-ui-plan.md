...
> **现状（2026-09-14，第9阶段演示版已落地、未提交）**：全局语音状态已经提升到老人端公共层，公共语音容器覆盖首页、事项、“我的”、助手、出行及记录二级页，页面切换不再卸载；助手页复用同一份会话与发送能力，没有保留第二套麦克风。除返回、重复朗读两种纯页面口令外，识别文本统一交给主智能体，前端只执行服务端返回的 `OPEN_TRAVEL` / `SHOW_INSIDE_GUIDE`，没有导航指令则进入助手展示同一轮回复，不从中文回答里猜页面或预约编号。单预约路线使用指令中的 `appointmentId`，归属仍由后端校验；多预约没有指令时进入助手澄清。录音开始会停止 TTS，发送用进行中锁防重复，并用递增请求序号阻止旧响应朗读或抢导航。前端类型检查、生产构建、临时端口启动与 HTTP 200 已通过；本轮未改后端、未改对外接口、未碰健康记录/备忘/家属报告。
>
> **尚未完成，不能写成已验收**：① 尚未带老人身份在真实浏览器逐页点击麦克风；② “看看我预约的地图”是否稳定选中 `travel.routePlan`、多预约是否稳定澄清尚未做真实模型测试；③ 当前页面、页面选中的预约和路线模式尚未作为结构化请求上下文传给后端；④ 请求乱序目前只有代码走查，没有自动化时序用例，旧响应虽不朗读、不导航，是否应继续写入对话显示仍需产品验收；⑤ 麦克风权限拒绝、识别失败、手机软键盘、安全区域、大字模式和真机浏览器兼容仍待检查；⑥ 队友负责的健康记录、备忘和家属报告尚未合并，当前只为它们保留统一语音入口，不声称语音业务已联调。
