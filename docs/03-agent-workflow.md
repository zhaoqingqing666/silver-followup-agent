...
- 表 `user_memories`，主键 `(user_id, memory_key)`；`MemoryStore` 负责读写，`digest()` 拼成一句话（**只服务「我的」页与既有测试，不再是主模型的画像入口**）。
- **写入口只有一处**：确认门禁放行、预约真的落进 `appointments` 之后，记下最近一次确认预约的医院、科室与时段（`habit.hospital` / `habit.department` / `habit.period`）。草稿阶段一个字都不记，模型和前端都够不着这条路径。
- **一次预约不是习惯**：写入措辞是「最近一次确认预约的医院是……」这类中性历史事实，`kind` 一律 `HISTORY`，不出现「常去」「习惯」。
- **读的时候不再自动进提示词**：`knownFacts` 里原来挂的 `memoryNote → digest` 已删除，默认上下文一条记忆都没有；模型要看画像，走只读工具 `profile.memorySummary` 按需查，结果带来源分类与更新时间。**记住不等于可以替他办事**，写操作仍要过确认门禁。
- 同一 key 只留最新一版（换医院就覆盖）；无记忆时 `digest()` 返回空串。
