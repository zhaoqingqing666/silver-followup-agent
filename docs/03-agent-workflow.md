...
- 取消预约的入口在事项页有两条：垃圾桶按钮走页内确认后直连 `POST /api/users/{userId}/appointments/{appointmentId}/cancel`，「让助手帮我取消」则把一句话交给助手页发出、走确认卡。两条都**先确认才写库**，且落到同一个业务方法（`AppointmentTool.cancel`）——**入口可以多，写路径只有一条**（见 DEC-020）。
