...
v0.2 已落地的增量能力：会话身份拆成“数据轴（服务谁）”与“能力轴（谁在操作）”两条轴，家属/志愿者可以代长辈办理；`ToolRegistry` 扩到 17 条工具（15 条只读 + 2 条确认交互），并按角色过滤模型可见的工具；新增健康记录（`HealthRecordTool`）与健康备忘（`MemoTool`）；协同照护端助手接入同一个智能体、同一套工具和同一个确认门禁。后端自动测试 317 项，0 失败 0 错误。身份、工具与协同照护端的细节见第十四、十五节。
...
- 已开放办事流程、医院、科室、号源、医生与号别、材料、已有预约、日程、路线、院内指引和药品知识等只读查询；`ToolRegistry` 目前共注册 20 条工具——**18 条只读**（其中 16 条两端通用，`care.timeline` 与 `care.notifications` 两条仅家属/志愿者可见），另有 **2 条确认交互**（`interaction.requestConfirmation`、`interaction.respondConfirmation`），只进确认状态机、不写业务数据。
...
助手页的「查看办理过程」已经实现了一轮之内的过程展示：按轮次列出「理解 → 决定查什么 → **模型提出的工具调用（参数是模型真实生成的那份）** → 工具真实返回 → 整理回答」。卡片默认收起，办理中也不自动展开，只在标题上换着显示此刻这一步，点开才看到参数与结果。这解决了一个具体的质疑——预先写好的对话也能宣称「调用了工具」，但参数由模型当场生成、工具当场返回，是没法预先编排的。
