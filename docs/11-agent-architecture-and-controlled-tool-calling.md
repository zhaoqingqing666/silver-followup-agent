...
当前支持一次提出最多三个互不依赖的只读工具；有依赖的查询先执行第一个，再让后续对话根据真实结果继续。普通聊天只调用一次主模型；查询轮次通常调用一次规划和一次工具结果回答，但两次复用同一个模型与核心提示词。模型不可用时保留规则流程作为运行降级，而不是正常模式的二次裁决。
...
当前允许一次规划提出最多三个互不依赖的只读工具；“复诊流程＋材料”可以同时查询 `careGuide.search` 与 `material.checklist`。有依赖的查询按模型给出的首个工具执行，真实结果由同一主模型继续回答。最多三个用于控制演示延迟和异常循环，不是业务规则。
...
- 扩展 `ModelRequest` 和 `ModelGateway`，支持工具定义、工具调用和工具结果。
- 建立 `ToolRegistry` 与 `ToolPolicy`。
- 第一批只开放医院、科室、号源、材料和已有预约查询。
- 每轮限制工具调用次数、超时和结果长度。
- 模型或工具失败时回到当前 Java 固定查询路线。

### 阶段三 引入统一智能体运行时

- 提取 `AgentRuntime` 管理模型与只读工具循环。
- 把 `FollowupAgentService` 缩小为 API 协调入口。
