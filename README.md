...
面向银发群体的复诊协同 Agent 比赛 Demo。仓库包含可交互的手机端页面、Java 任务流、H2 模拟数据库、一套在 `ToolRegistry` 注册、每次调用都留痕的模拟工具，以及团队协作文档。

> 所有医院、用户、号源、路线和通知均为模拟数据。本项目不提供疾病诊断或用药建议。

需求基线见 [Requirement.md](Requirement.md)。当前设计、源码与需求差距、待实现方案和验收优先级见 [Design.md](Design.md)——它是 2026-09-07 的审阅基线，其中列出的缺口有些此后已经补上（紧急暂停、确认版本绑定、部分失败恢复），哪一条还没实现以 `docs/records/PROGRESS.md` 的最新记录为准。已有主流程不代表全部要求已验收。
...
前端先选身份（就诊人本人 / 家属 / 志愿者），就诊人端包含首页、复诊助手、复诊事项卡、出行与院内指引、健康记录与备忘和适老设置，家属与志愿者进入共用的协同照护端。助手默认允许自然交流；用户明确表达预约复诊目标后才创建可暂停、可恢复的后台办理任务。对话过程中按需展示任务状态、计划卡、异常卡、明确确认卡和最终事项卡，「查看办理过程」按轮次显示模型当场生成的工具参数与工具真实返回。后端执行“理解需求 → 补问缺失信息 → 查询号源 → 日程检查 → 计划出行 → 用户确认 → 提交预约 → 创建提醒 → 通知家属”，写操作一律经过确认门禁。当前版本的功能范围、验证结论与限制见 [docs/](docs/) 与 [docs/records/PROGRESS.md](docs/records/PROGRESS.md)。
...
不配置模型时，项目自动使用本地规则和回答模板；只读工具与业务写入仍会真实执行，识图、云端语音输入和云端朗读这三项会关闭或回落到浏览器原生能力。启用后，`ConversationPlanner` 可以提出普通回答、自然补问、只读工具调用或工作流动作；`AgentRuntime` 与 Java 权限策略审核后才进入真实工具和业务状态机，最终回答仍受权威工具结果约束。
...
本文件只讲怎么跑起来和团队怎么配合；当前版本做了什么、验证到什么程度、还有哪些限制，看 `docs/records/PROGRESS.md` 与 `docs/报名解决方案.md` 的迭代计划。
