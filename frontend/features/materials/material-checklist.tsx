...
export function MaterialChecklist({ appointmentId, disabled = false, compact = false }: {
  ...
  const done = materials.filter(item => item.status !== MATERIAL_STATUS.NOT_PREPARED).length;
  ...
  return <section className={compact ? 'mt-4 rounded-2xl bg-white/70 p-4' : 'rounded-3xl border bg-card p-5 shadow-sm'}>
    ...
    <div className="mt-3 divide-y">
      {materials.map(item => {
        const prepared = item.status !== MATERIAL_STATUS.NOT_PREPARED;
        ...
        return <button key={item.id} disabled={disabled || !!updatingId} onClick={() => void toggle(item)} className="flex min-h-14 w-full items-center gap-3 py-3 text-left disabled:opacity-60">
          ...
          {item.status === MATERIAL_STATUS.PHOTO_CONFIRMED && <span className="ml-auto text-sm text-green-700">拍照确认</span>}
          {item.required && item.status !== MATERIAL_STATUS.PHOTO_CONFIRMED && <span className="ml-auto text-sm text-primary">必带</span>}
        </button>;
      })}
