...
function VoiceBubble({ message }: { message: ChatMessage }) {
  ...
  return (
    <>
      <button type="button" onClick={togglePlay} aria-label={playing ? '停止播放语音' : '播放语音'} style={{ width: widthPx }} className="flex min-h-12 items-center justify-between gap-2 rounded-2xl bg-primary px-4 text-primary-foreground">
    ...
    <div className={`max-w-[88%] space-y-2 rounded-3xl px-4 py-3 text-[17px] leading-7 ${message.role === 'user'
      ...
      {message.role === 'user' && message.audioUrl && (
        <>
          ...
          {message.text && (
            <p className="flex items-start gap-1.5"><span className="shrink-0 text-base leading-6" title="语音识别">🎤</span><span>{message.text}</span></p>
          )}
          {message.voiceState === 'transcribing' && <p className="flex items-center gap-1.5 text-xs opacity-70"><LoaderCircle className="size-3 animate-spin" />正在识别…</p>}
      ...
      {message.role === 'assistant' && message.text && (
        <>
          ...
          {/* pre-line：模型回复里的换行（"重要信息单独成行"）要真的换行显示，而不是挤成一行 */}
          <p className="whitespace-pre-line">{message.text}</p>
        </>
      )}

      {message.role === 'assistant' && message.text && (
        <button onClick={() => void speak()} disabled={loading} aria-label={playing ? '停止朗读' : '朗读这条回复'} className="mt-1 inline-flex size-9 items-center justify-center rounded-full border border-[#e8c7a4] bg-white text-primary shadow-sm">
