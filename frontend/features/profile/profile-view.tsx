...
interface ProfileProps { onNavigate: (tab: TabId) => void; largeText: boolean; onLargeTextChange: (value: boolean) => void }
export function ProfileView({ onNavigate, largeText, onLargeTextChange }: ProfileProps) {
  return <main className="space-y-5 px-5 pb-8 pt-5">
    <PageHeader title="我的" onBack={() => onNavigate('home')} />
    <section className="flex items-center gap-4 rounded-3xl bg-gradient-to-r from-[#f3c78f] to-[#f8dfbd] p-5"><div className="grid size-16 place-items-center rounded-full bg-white/75"><UserRound className="size-8 text-primary" /></div><div><h2 className="text-2xl font-bold">张阿姨</h2><p className="text-base text-muted-foreground">家属联系人：女儿 · 138****1234</p></div></section>
    <section className="overflow-hidden rounded-3xl border bg-card shadow-sm">
      ...
      <div className="flex min-h-18 items-center gap-4 border-b px-5"><Headphones className="size-7 text-primary"/><div className="flex-1"><strong className="text-lg">语音朗读</strong><p className="text-sm text-muted-foreground">智能体回复后自动朗读</p></div><Switch defaultChecked aria-label="语音朗读" /></div>
