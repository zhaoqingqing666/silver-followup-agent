...
export async function cancelAppointment(appointmentId: string, userId = DEMO_USER_ID): Promise<boolean> {
  const response = await fetch(`${API_BASE}/api/users/${userId}/appointments/${appointmentId}`, { method: 'DELETE' });
...
export async function getVoicePreference(userId = DEMO_USER_ID): Promise<VoicePreference> {
  ...
  if (!response.ok) throw new Error('无法读取语音偏好');
  return response.json();
}
