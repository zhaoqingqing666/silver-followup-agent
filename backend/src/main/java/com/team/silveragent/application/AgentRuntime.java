...
final class AgentRuntime {
    record Outcome(AgentOrchestrator.Route route, ExtractedFacts facts, String replyDraft,
                   ...
                   boolean javaFallback) {

        /**
         * 兼容构造：绝大多数路由没有结构化推荐，也不单独给最终话语，更不需要退回 Java 链路。
         *
         * <p>留这一条是为了让「模型这轮说了什么」的既有构造点一处都不用改——
         * 只有 {@code modelProposal} 与 {@code continueAfterTools} 要把新读出来的值传下去。
         *
         * <p>{@code javaFallback} 默认 {@code false}：这一轮按算出来的路由正常办，
         * 不退回 Java 关键词链路；只有「模型给了话但这话不能用」的出口才传 {@code true}。
         */
        Outcome(AgentOrchestrator.Route route, ExtractedFacts facts, String replyDraft,
            ...
            this(route, facts, replyDraft, dialogueMode, plannerSource, proposedTool, proposedTools,
                    actionType, intent, modelDriven, List.of(), null, false);
        }
    ...
    private Outcome rejectedAnswer(PlannerDecision proposal) {
        return new Outcome(AgentOrchestrator.Route.CURRENT_FLOW, proposal.facts(), null,
                normalizeDialogueMode(proposal.dialogueMode()), proposal.source(), null, List.of(),
                proposal.actionType(), proposal.intent(), true,
                proposal.recommendations(), proposal.answering(), true);
    }
    ...
    private Outcome outcome(AgentOrchestrator.Route route, PlannerDecision proposal,
        ...
        return new Outcome(route, proposal.facts(), proposal.replyDraft(),
                ...
                proposal.recommendations(), proposal.answering(), false);
    }
